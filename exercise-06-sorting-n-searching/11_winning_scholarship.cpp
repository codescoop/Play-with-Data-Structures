...
bool willGetScholarship(ll totalStudents, ll totalCoupons, ll minReqCoupons, ll couponAmountFromStudents, ll givenStudents)
{
    ll requiredCoupons = givenStudents*minReqCoupons;
    ...
    {
        ll pendingStudents = totalStudents - givenStudents;
        ll additionalCouponsFromStudents = pendingStudents*couponAmountFromStudents;

        ll availableCoupons = totalCoupons + additionalCouponsFromStudents;
...
ll getMaxStudents(ll totalStudents, ll totalCoupons, ll minReqCoupons, ll couponAmountFromStudents)
{
    /* 
    ...
    */
    ...
    ll min = totalCoupons/minReqCoupons;

    ll start = min;
    ll end = totalStudents;
    ll students = INT_MIN;

    while(start <= end){
        ll mid = (start+end)/2;
        // check if it is possible to given scholarship to x students
        ll status = willGetScholarship(totalStudents, totalCoupons, minReqCoupons, couponAmountFromStudents, mid);
        if(status){
...
int main()
{
	ll totalStudents, totalCoupons, minReqCoupons, couponAmountFromStudents;
    ...
    ll students = getMaxStudents(totalStudents, totalCoupons, minReqCoupons,couponAmountFromStudents);
